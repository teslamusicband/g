...
add_key_to_authorized_keys() {
    ...
    # Проверяем длину ключа перед обработкой
    log "    Длина полученного ключа: ${#public_key} символов"
    
    # Создаем локальный временный файл для безопасной передачи
    local local_temp_file="/tmp/temp_key_$(date +%s)_$.txt"
    echo "$public_key" > "$local_temp_file"
    
    # Проверяем, что ключ записался полностью
    local saved_key=$(cat "$local_temp_file")
    log "    Длина сохраненного ключа: ${#saved_key} символов"
    
    if [[ ${#saved_key} -ne ${#public_key} ]]; then
        error "    Ключ был обрезан при сохранении!"
        rm -f "$local_temp_file"
        return 1
    fi
    
    # Передаем ключ через stdin, а не через аргументы командной строки
    local remote_script='
        # Читаем ключ из stdin
        read -r PUBLIC_KEY
        
        # Проверяем длину полученного ключа
        echo "RECEIVED_KEY_LENGTH:${#PUBLIC_KEY}"
    ...
    # Выполняем команду, передавая ключ через stdin
    local result
    result=$(cat "$local_temp_file" | sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=10 "$USERNAME@$server" "$remote_script" 2>/dev/null)
    
    # Удаляем временный файл
    rm -f "$local_temp_file"
    
    # Проверяем результат
    if [[ "$result" == *"RECEIVED_KEY_LENGTH:"* ]]; then
        local received_length=$(echo "$result" | grep "RECEIVED_KEY_LENGTH:" | cut -d: -f2)
        log "    Сервер получил ключ длиной: $received_length символов"
        
        if [[ "$received_length" -ne "${#public_key}" ]]; then
            error "    Ключ был обрезан при передаче на сервер!"
            return 1
        fi
    fi
...
distribute_keys() {
    ...
    # Проверяем содержимое файла с ключами
    if [[ ! -f "$keys_file" ]]; then
        error "Файл с ключами не найден: $keys_file"
        return 1
    fi
    
    log "Содержимое файла с ключами ($keys_file):"
    while IFS= read -r line; do
        if [[ -n "$line" ]]; then
            log "  Ключ: ${line:0:80}... (длина: ${#line} символов)"
        fi
    done < "$keys_file"
    ...
    while IFS= read -r public_key || [[ -n "$public_key" ]]; do
        ...
        log "Обработка ключа #$line_number (длина: ${#public_key} символов)"
        log "  Начало ключа: ${public_key:0:50}..."
...
# Очистка некорректных записей в known_hosts
cleanup_known_hosts() {
    log "Проверка и очистка файлов known_hosts..."
    
    for server in "${SERVERS[@]}"; do
        log "Очистка known_hosts на $server..."
        
        local cleanup_script='
            # Проверяем наличие некорректных записей в known_hosts
            if [[ -f ~/.ssh/known_hosts ]]; then
                echo "BEFORE_CLEANUP:$(wc -l < ~/.ssh/known_hosts)"
                
                # Создаем резервную копию
                cp ~/.ssh/known_hosts ~/.ssh/known_hosts.backup.$(date +%s) 2>/dev/null || true
                
                # Удаляем строки, которые выглядят как публичные ключи вместо host keys
                # Публичные ключи обычно начинаются с ssh-rsa, ssh-ed25519 и содержат длинные строки
                # но не должны быть в known_hosts
                grep -v "^[a-zA-Z0-9.-]* ssh-[a-z0-9]* AAAAC3Nza.*$" ~/.ssh/known_hosts > ~/.ssh/known_hosts.tmp 2>/dev/null || true
                
                if [[ -f ~/.ssh/known_hosts.tmp ]]; then
                    mv ~/.ssh/known_hosts.tmp ~/.ssh/known_hosts
                    chmod 600 ~/.ssh/known_hosts
                fi
                
                echo "AFTER_CLEANUP:$(wc -l < ~/.ssh/known_hosts)"
            else
                echo "NO_KNOWN_HOSTS_FILE"
            fi
            
            # Проверяем authorized_keys
            if [[ -f ~/.ssh/authorized_keys ]]; then
                echo "AUTHORIZED_KEYS:$(wc -l < ~/.ssh/authorized_keys)"
                
                # Показываем первые символы каждого ключа для проверки
                while IFS= read -r line; do
                    if [[ -n "$line" ]]; then
                        echo "AUTH_KEY_PREVIEW:${line:0:80}... (length: ${#line})"
                    fi
                done < ~/.ssh/authorized_keys
            else
                echo "NO_AUTHORIZED_KEYS_FILE"
            fi
        '
        
        if result=$(execute_remote_command "$server" "$cleanup_script" 2>/dev/null); then
            log "  Результат для $server:"
            echo "$result" | while IFS= read -r line; do
                case "$line" in
                    BEFORE_CLEANUP:*)
                        log "    Known_hosts до очистки: ${line#BEFORE_CLEANUP:} строк"
                        ;;
                    AFTER_CLEANUP:*)
                        log "    Known_hosts после очистки: ${line#AFTER_CLEANUP:} строк"
                        ;;
                    AUTHORIZED_KEYS:*)
                        log "    Authorized_keys: ${line#AUTHORIZED_KEYS:} ключей"
                        ;;
                    AUTH_KEY_PREVIEW:*)
                        log "    ${line#AUTH_KEY_PREVIEW:}"
                        ;;
                    NO_KNOWN_HOSTS_FILE)
                        log "    Файл known_hosts отсутствует"
                        ;;
                    NO_AUTHORIZED_KEYS_FILE)
                        log "    Файл authorized_keys отсутствует"
                        ;;
                esac
            done
        else
            error "  Не удалось выполнить очистку на $server"
        fi
    done
}
...
cleanup() {
    ...
    rm -f /tmp/temp_key_*.txt 2>/dev/null || true
}
