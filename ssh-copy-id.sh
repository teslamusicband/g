...
get_public_key_from_server() {
    ...
    execute_remote_command "$server" "cat ~/.ssh/id_rsa.pub 2>/dev/null || echo 'NO_KEY_FOUND'"
}
...
collect_public_keys() {
    ...
    for server in "${SERVERS[@]}"; do
        ...
        local public_key=$(get_public_key_from_server "$server")
...
distribute_keys() {
    ...
    while IFS= read -r public_key; do
        ...
        log "Распространение ключа: ${public_key:0:50}..."
...
cleanup() {
    ...
    rm -f /tmp/ssh_keys_* 2>/dev/null || true
}
